...
- [x] Handler `cmd_zhk` — аргумент: название или id ЖК
- [x] Без аргумента — список ЖК с активными объявлениями (топ-10 по количеству)
- [x] С аргументом — последний снимок: медиана, средняя, активных, продано за 30д, дней на рынке
- [x] Формат HTML, компактный блок
...
- [x] Handler `cmd_report`
- [x] Данные: новых за 7д, снято за 7д, топ-3 ЖК по дисконту, топ-3 по ликвидности (sold_last_30d)
- [x] Опционально: `/report 7` / `/report 30` — период в днях
- [x] Генерация текста в `app/analyzer/report_builder.py`
...
- [x] Handler `cmd_vip`
- [x] JOIN `apartments` + `apartment_scores` + фильтр `apply_search_filters`
- [x] Сортировка: grade (A+ first), score DESC, discount_pct DESC
- [x] Лимит 5 карточек
- [x] Показать: ЖК, цена, дисконт, grade, ROI, ссылка
...
- [x] `app/scheduler/hunter_job.py` — логика охотника
- [x] Интервал: 30 мин (настраивается `HUNTER_INTERVAL_MINUTES`)
- [x] Условия алерта: новое за последний цикл + (мотивация ИЛИ grade ≥ A) + дисконт ≥ 10%
- [x] Дедупликация через `notifications` (не слать повторно по apartment_id)
- [x] Отдельный тип уведомления `hunter_alert`
- [x] Не дублировать обычные post-scrape уведомления
...
- [x] `/settings` — inline-кнопки: «Цена до», «Комнаты», «Площадь», «Текст»
- [x] ConversationHandler или callback query flow
- [x] Валидация ввода (числа, диапазоны)
- [x] `search_config_repo.update_field(config_id, field, value)`
- [x] Подтверждение после сохранения + превью URL-параметров
...
- [x] Шаблон: ЖК, цена, площадь, дисконт, ссылка, рекомендация
- [x] `recommendation` из `apartment_scores` или генерация в `InvestmentScorer`
- [x] Разные шаблоны: `new_deal`, `hunter_alert`, `price_drop`
- [x] Запись в `notifications` с полным текстом
...
- [x] Фильтры можно менять через `/settings` без правки БД вручную
- [x] Уведомления содержат рейтинг, ROI и текст рекомендации
