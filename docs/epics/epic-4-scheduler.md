...
- [ ] `app/scheduler/scheduler.py` — создание и настройка планировщика
- [ ] Планировщик стартует в `lifespan` FastAPI (не в `@app.on_event`)
- [ ] При завершении приложения — корректный shutdown планировщика
- [ ] Job store: memory (для MVP, без персистентности)
- [ ] Endpoint GET `/scheduler/status` — список задач и время следующего запуска
...
- [ ] `app/scheduler/jobs.py` — функция `scrape_job()`
- [ ] Job регистрируется в планировщике с интервалом из конфига
- [ ] Защита от параллельного запуска (если предыдущий ещё идёт — пропустить)
- [ ] Логирование старта, итогов и ошибок каждого цикла
- [ ] Результат `ScrapeResult` логируется в структурированном виде
