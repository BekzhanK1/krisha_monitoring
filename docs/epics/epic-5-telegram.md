...
**Следующий эпик:** [Epic 6 — Аналитика (V2)](epic-6-analytics.md)
...
- [x] `app/telegram/bot.py` — создание `Application` и `Bot`
- [x] Бот запускается в lifespan (polling или webhook)
- [x] `app/telegram/sender.py` — функция `send_message(text, parse_mode="HTML")`
- [x] Обработка ошибок отправки (retry 3 раза)
- [x] Логирование каждой отправки
...
- [x] `app/analyzer/deal_analyzer.py` — класс `DealAnalyzer`
- [x] Метод `get_market_stats(complex_id) -> MarketStats` — средняя/медианная цена
- [x] Метод `find_deals(complex_id) -> list[Deal]` — квартиры с дисконтом
- [x] Критерии из ТЗ: цена ниже медианы на 10%, цена/м² ниже на 10%, ТОП-5
- [x] Расчёт `discount_pct` — процент дисконта от медианы
- [x] Определение мотивированных продавцов по тексту описания
...
- [x] Интеграция `DealAnalyzer` в `scrape_job`
- [x] Отправлять уведомление только для **новых** выгодных квартир (не повторять)
- [x] Поле `notifications` в БД — фиксировать отправленные уведомления
- [x] Формат сообщения из ТЗ (раздел 8)
- [x] Не отправлять дубли: проверять `notifications` перед отправкой
...
- [x] `/top` — ТОП-10 самых дешёвых квартир по цене/м² прямо сейчас
- [x] `/new` — квартиры, найденные за последние 24 часа
- [x] `/discount` — квартиры с дисконтом >10% от медианы
- [x] Ответ: список карточек, максимум 5 объявлений в одном сообщении
- [x] Если нет данных — понятный текст "Пока нет данных"
- [x] Обработка ошибок — не падать, отвечать "Ошибка, попробуйте позже"
